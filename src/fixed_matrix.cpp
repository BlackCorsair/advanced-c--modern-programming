...
public:
    fixed_matrix(std::initializer_list<std::initializer_list<T>> matrix) {
       ...
       // std::copy(matrix.begin()->begin(), matrix.end()->end(), &mat_[0][0]);
    }
...
void test1() {
    fixed_matrix<double,3,4> a{{1,1,2}, {2,2,2} }; std::cout << "matrix 1\n";
    print(a);
    std::cout << std::endl;
}

void test2() {
    fixed_matrix<double,3,4> a{{1,1,2}, {2,2,2}, {1}, }; std::cout << "matrix 2\n";
    print(a);
    std::cout << std::endl;
}
